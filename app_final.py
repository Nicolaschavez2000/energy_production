...
def plot_energy_distribution(region, data):
    ...
    fig.update_layout(width=1400,height=550,margin=dict(t=50, b=50, l=0, r=50))
    return fig
...
def prophet_energias(data, target_year=2030):
    ...
    color_map = {
        'nuclear': {'historical': '#7E57C2', 'prediction': '#5E35B1'},
        'petroleo': {'historical': '#EF5350', 'prediction': '#D32F2F'},
        'solar': {'historical': '#FFD54F', 'prediction': '#FFC107'},
        'eolica': {'historical': '#66BB6A', 'prediction': '#43A047'},
        'gas': {'historical': '#42A5F5', 'prediction': '#1E88E5'},
        'hidro': {'historical': '#29B6F6', 'prediction': '#0288D1'},
        'bio fuel': {'historical': '#8D6E63', 'prediction': '#5D4037'},
        'carbon': {'historical': '#616161', 'prediction': '#212121'}
    }
        ...
        if c in produccion.columns and c in consumo.columns:
            ...
            if not energy_future_forecast.empty:
                ...
                fig_energia.add_trace(go.Scatter(
                    ...
                    mode='lines',
                    line=dict(color=prediction_color, dash='dash', width=1),
                    showlegend=False,
                    legendgroup=c
                ))
